...
class TimedScriptAppUserBind extends Command
{
    ...
    public function app_handle( $app_id )
    {
        ...
        //点击数据
        $click_datas = $AppClickDataM->select("id", "unique_id", "platform_id", "imei", "idfa", "androidid", "oaid", "mac", "ip", "os", "ts", "ua", "callback_url")->whereBetween( "ts", [$first_time * 1000, $last_time * 1000] )->orderBy('id', 'desc')->limit( $data_limit )->get()->toArray();
